...
from iec_60870_server import iec_60870_Server


@pytest.fixture
def iec_60870_class():
    return iec_60870_Server()

def test_get_ip_address_success(iec_60870_class):
    ...
    with patch('socket.socket') as mock_socket:
        ...
        ip_address = iec_60870_class.get_ip_address()

        assert ip_address == '192.168.100.3'
        mock_instance.connect.assert_called_with(("8.8.8.8", 80))
        mock_instance.close.assert_called_once()

def test_get_ip_address_socket_error(iec_60870_class):
...
def test_send_response_valid_request(iec_60870_class):
    iec_60870_class.gramophile_dict = {
        "010203": "040506"
    }
    response = iec_60870_class.send_response("01 02 03")
    assert response == b'\x04\x05\x06'

def test_send_response_invalid_request(iec_60870_class):
    iec_60870_class.gramophile_dict = {
        "010203": "040506"
    }
    response = iec_60870_class.send_response("FF EE DD")
    assert response == ""

def test_send_response_non_hex_input(iec_60870_class):
    iec_60870_class.gramophile_dict = {
        "010203": "040506"
    }
    response = iec_60870_class.send_response("G1 H2 I3")
    assert response == ""
    


# def test_socket_connection():
#     server = iec_60870_Server()
