...
class ModbusServer:
    def __init__(self):
        ...
        self.PORT = 502  # Port to listen on (non-privileged ports are > 1023)
        self.gramophile_dict = {}

        self.csv_file = "/home/ubuntu/protocols/modbus_protocol/Modbus_Gramophile.csv"
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                ...
                try:
                    ...
                    self.data = self.client_socket.recv(1024)  # Receive up to 1024 bytes
                    self.data.decode('utf-8')
                    print(self.data)
                    print(" ==========================================================================================================================================================================")
                    print(self.data)
                    logging.info(f"Request recieved from {self.client_address} : {self.data}")
                    if not self.data:
                    ...
                    self.result_hex = " ".join(format(ord(byte), '02x') for byte in unescaped_bytes)
                    print('Recieved message: ' + self.result_hex)
                    # Send a response back to the client (optional)
                    response = self.send_response(self.result_hex)
                    print(response,' ------------ response')
                    logging.info(f"Response send to {self.client_address} : {response}")
