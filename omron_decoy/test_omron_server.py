...
from omron_server import omronServer
...
def test_get_ip_address_success(omron_class):
...
def test_send_response_valid_request(omron_class):
    omron_class.gramophile_dict = {
        "010203": "040506"
    }
    response = omron_class.send_response("01 02 03")
    assert response == b'\x04\x05\x06'

def test_send_response_invalid_request(omron_class):
    omron_class.gramophile_dict = {
        "010203": "040506"
    }
    response = omron_class.send_response("FF EE DD")
    assert response == ""

def test_send_response_non_hex_input(omron_class):
    omron_class.gramophile_dict = {
        "010203": "040506"
    }
    response = omron_class.send_response("G1 H2 I3")
    assert response == ""
