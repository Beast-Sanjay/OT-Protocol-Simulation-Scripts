...
from pcomtcp_server import PCOMServer
...
def test_get_ip_address_success(pcomtcp_class):
    ...
    with patch('socket.socket') as mock_socket:
        ...
        ip_address = pcomtcp_class.get_ip_address()

        assert ip_address == '192.168.100.3'
        mock_instance.connect.assert_called_with(("8.8.8.8", 80))
        mock_instance.close.assert_called_once()

def test_get_ip_address_socket_error(pcomtcp_class):
...
def test_send_response_valid_request(pcomtcp_class):
    pcomtcp_class.gramophile_dict = {
        "010203": "040506"
    }
    response = pcomtcp_class.send_response("01 02 03")
    assert response == b'\x04\x05\x06'

def test_send_response_invalid_request(pcomtcp_class):
    pcomtcp_class.gramophile_dict = {
        "010203": "040506"
    }
    response = pcomtcp_class.send_response("FF EE DD")
    assert response == ""

def test_send_response_non_hex_input(pcomtcp_class):
    pcomtcp_class.gramophile_dict = {
        "010203": "040506"
    }
    response = pcomtcp_class.send_response("G1 H2 I3")
    assert response == ""
    


# def test_socket_connection():
#     server = pcomtcpServer()
